...
public class AssignmentData extends Datasource {
    ...
    public void assignPatient(int roomNumber, String patientName) throws IOException, SQLException {
        try (Connection conn = DriverManager.getConnection(getConnectionString());
             ...
             ResultSet results = statement.executeQuery("SELECT id " +
                                                            "FROM patients " +
                                                            "WHERE name = '" + patientName + "'")) {
            int patientID = results.getInt(1);
            statement.execute("UPDATE rooms " +
                                  "SET assignedPatient = " + patientID + " " +
    ...
    public String getAssignedPatient(int roomNumber) throws IOException, SQLException {
        try (Connection conn = DriverManager.getConnection(getConnectionString());
             Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery("SELECT assignedPatient " +
                                                            "FROM rooms " +
                                                            "WHERE roomNumber = " + roomNumber)) {
            ...
            ResultSet assignedPatientResults = statement.executeQuery("SELECT name " +
                                                                          "FROM patients " +
                                                                          "WHERE id = " + assignedPatientID);
    ...
    public String getAssignedNurse(int roomNumber) throws IOException, SQLException {
        try (Connection conn = DriverManager.getConnection(getConnectionString());
             Statement statement = conn.createStatement();
             ResultSet results = statement.executeQuery("SELECT assignedNurse " +
                                                            "FROM rooms " +
                                                            "WHERE roomNumber = " + roomNumber)) {
            ...
            ResultSet assignedNurseResults = statement.executeQuery("SELECT name " +
                                                                        "FROM nurses " +
